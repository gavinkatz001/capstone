...
struct globpar 
  ...
  int (*lecallback)(int, int, int, unsigned char*, int);
              // screen print buffer 
...
int init_blue_ex(char *filename,int hcin)
  ...
  int clflag,leflag,readret,meshcount,reportflag;
  unsigned int ind[16];
  struct cticdata *cp;
...
int devlist(int mask)
  {
  int n,i,j,k,xn,count,flag,maxlen,node;
  int vn[NUMDEVS],len[NUMDEVS];
  char *s,buf[64];
  struct devdata *dp;
...
int le_server(int(*callback)(int,int,int,unsigned char*,int),int timerds)
  {
    ...
    if(n >= 0)
      ...
      if(op == LE_READ)
        ...
        if(callback != NULL)
          ...
          if(op == LE_WRITE)
            {
            // Extract raw data from the instruction stack
            int nread = instack[n+1] + (instack[n+2] << 8) - 2;
            retval = callback(dp->node,op,cticn,insdatn + 2,nread);
            }
          else
            {
            // For other operations, pass NULL and 0 for data
            retval = callback(dp->node,op,cticn,NULL,0);
            }
          }
        cbflag = 1;
#endif
...
#else
        if(callback != NULL)
          retval = callback(localnode(),LE_TIMER,0,NULL,0);
#endif
        tim0 = timems();
        }
      }
...
#else
        if(callback != NULL)
          retval = callback(localnode(),LE_KEYPRESS,key,NULL,0);
#endif
        key = 0;
        }
      }  
...
int universal_server(int(*callback)(int,int,int,unsigned char*,int),char endchar,int keyflag,int timerds)
  ...
  int node,op,cticn;
  unsigned char readret,buf[1024];
  unsigned long long tim0,timms;
  struct devdata *dp;
#ifdef BTFPYTHON
  PyObject *pycallback;
...
int leserver(int ndevice,int count,unsigned char *dat,int insn)
  ...
  unsigned char cmd[4],*s,*data,errcode,buf[32];
  struct cticdata *cp;
  ...
  if(dat[0] == 0x52 || dat[0] == 0x12 || dat[0] == 0x0A)
          ...
          if(notflag == 0)
            ...
            if((cp->perm & 2) == 0)
              ...
              if(gpar.lecallback != NULL)
                cmd[2] = (unsigned char)gpar.lecallback(dev[ndevice]->node,LE_READ,cticn,NULL,0);
#endif
    ...
    if(errcode == 0 && (notflag == 0 || notflag == 1) && flag != 0 && cmd[0] != 0)
      ...
      if(cmd[0] == LE_WRITE)
        {
        // Include raw data for LE_WRITE operations
        unsigned char *cmdwithdata = malloc(3 + datcount);
        if(cmdwithdata != NULL)
          {
          cmdwithdata[0] = cmd[0];
          cmdwithdata[1] = cmd[1];
          cmdwithdata[2] = cmd[2];
          memcpy(cmdwithdata + 3, data, datcount);
          pushins(IN_LECMD,ndevice,3 + datcount,cmdwithdata);
          free(cmdwithdata);
          }
        else
    ...
    NPRINT "%s handle READ/WRITE IS %04X\n",dev[ndevice]->name,handle);
    if(flag == 0 && aflag == 0)
      {
